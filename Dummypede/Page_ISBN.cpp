...
void Page_ISBN::Draw() const{
	...
	if(page_name == U"ISBN Code") {
		...
		FontAsset(U"HugeMintyo")(U"9784XXXXXXXXX").drawAt(size.x/2, base.y + 200, Palette::Black);
		...
		FontAsset(U"RegularThinMintyo")(U"13桁のISBN Codeにより書籍の検索が可能。").draw(base + Vec2(10, 407), Palette::Black);
		...
		*/
		...
		RectF(base.movedBy(0, 225), Vec2(460, 195)).drawFrame(4, Palette::Red);
		RectF(base.movedBy(0, 225), Vec2(460, 30)).draw(Palette::Red);
		FontAsset(U"BoldRegular")(U"寄付のお願い（ご一読ください）").draw(base + Vec2(10, 225), Palette::White);
		FontAsset(U"RegularThinMintyo")(U"Mikipediaは多くの支援により成立していますが、\n依然として厳しい資金難に直面しています。\nMikipediaを、あなたの力で支えませんか。").draw(base + Vec2(10, 260), Palette::Black);
		...
		FontAsset(U"RegularThinMintyo")(U"寄付のご協力ありがとうございます。").draw(base + Vec2(10, 380), Palette::Black);
	}
...
Page_ISBN::Page_ISBN(WindowSystemManager& manager, String const & str)
	:WindowSystem(Vec2(350, 150), Size(500, 520), str + U" - Mikipedia"),
	manager(manager),
	page_name(str),
	text(U""),
	textarea_rect(Vec2(240, 60), Size(220, 30)),
	textarea_active(true),
	donate_button(Vec2(120, 414), Vec2(250, 35)),
